...
class OwenProtocol:  # Класс, реализующий протокол ОВЕН напрямую в пайтон
    ...
    def __str__(self):
        return ('Address: {} Address length: {}\nHash: {} Request: {}\nData size: {} Data: {}\n' + \
                'Frame: {}\nFrame in ascii: {}\nCrc: {} Crc is OK: {}').format(self.address, self.addrLen, self.hash,
                                                                               ...
                                                                               list(self.data), \
                                                                               list(self.frame), self.frameAscii,
                                                                               self.crc, self.crcOk)
    ...
    def unpackAsciiFrame(self):  # преобразует данные, полученные от устройства, в бинарный вид
        ...
        if not (self.frameAscii[0] == '#' and self.frameAscii[-1] == '\x0d'):
            raise OwenProtocolError(
                'OwenProtocol::ascii buffer does not have start or stop bytes! Frame ascii: {}'.format(
                    list(self.frameAscii)))
        if not ((frameAsciiSize & 1) == 0):
            raise OwenProtocolError(
                'OwenProtocol::ascii buffer has not even length! Frame ascii: {}'.format(list(self.frameAscii)))
        i = j = 0
        self.frame = ''
    ...
    def unpackIEEE32(self, withIndex=False, withTime=False):  # извлекает из данных число с плаваЮщей точкой и время
        ...
        if not (self.dataSize == 4 + additionalBytes):
            raise OwenUnpackError(
                'OwenProtocol::wrong size of data ({0}) when IEEE32 unpacking, should be {1}!'.format(self.dataSize, (
                        4 + additionalBytes)), \
                self.data)
    ...
    def unpackFloat24(self, withIndex=False, withTime=False):  # извлекает из данных число с плаваЮщей точкой
        if not (self.dataSize == 3):
            raise OwenUnpackError(
                'OwenProtocol::wrong size of data ({0}) when float24 unpacking!'.format(self.dataSize), self.data)
        value = struct.unpack('>f', self.data[0:3] + '\x00')[0]
        result = dict(value=value, time=-1, index=-1)
        return result

    def unpackBCD(self, withIndex=False, withTime=False):  # извлекает из данных BCD
        # if not (self.dataSize == 3):
        #     raise OwenUnpackError(
        #         'OwenProtocol::wrong size of data ({0}) when float24 unpacking!'.format(self.dataSize), self.data)
        # value0 = struct.unpack('>f', self.data[0:4])[0]
        # value = struct.unpack('b', self.data[0])[0]

        signbit = ord(self.data[0]) & 0x80
        if signbit:
            sign = -1
        else:
            sign = 1
        point = ord(self.data[0]) & 0x70
        point = point >> 4
        val = ord(self.data[0]) & 0x0F
        for i in range(1, 4):
            val = val * 10
            temp1 = ord(self.data[i]) & 0xF0
            temp2 = temp1 >> 4
            val = val + temp2
            val = val * 10
            val = val + (ord(self.data[i]) & 0x0F)
    ...
    def unpackInt16(self, withIndex=False, withTime=False):  # извлекает из данных целое число со знаком
        if not (self.dataSize >= 1):
            raise OwenUnpackError(
                'OwenProtocol::wrong size of data ({}) when unsigned short unpacking!'.format(self.dataSize), self.data)
    ...
    def unpackChar(self):  # извлекает из данных байт со знаком, возвращает целый тип
        if not (self.dataSize == 1):
            raise OwenUnpackError('OwenProtocol::wrong size of data ({0}) when char unpacking!'.format(self.dataSize),
                                  self.data)
        value = struct.unpack('b', self.data[0])[0]
        result = dict(value=value, time=-1, index=-1)
        return result
    ...
    def packFrame(self,
                  needDataSize=-1):  # формирует массив байтов из данных класса для передачи на устройство, возвращает фрейм в виде строки
        frame = []
        # адрес
        if self.addrLen == 8:
...
class NetworProtocolSettings:
    def __init__(self, baudRateI=0, lenI=0, parityI=0, stopBitsI=0, address=0, addressLengthI=0, errorNumber=0,
                 answerDelay=0):
    ...
    def __str__(self):
        return ('Baud rate: {} Bit length: {} Parity: {} Stop bits: {}\n' + \
                'Base address: {} Address length: {} Aswer delay: {} Last error: {}').format(self.baudRate, self.bits,
...
class OwenDevice:
    ...
    def __init__(self, serialPort=None, address=16, addrLen=8):
        ...
        self.__TestResponseStringForIEEE32 = "#HOGMONOKSIKNVVQNGVUHTRGK" + chr(
            13)  # эта строка будет "читаться" из устройства, если serialPort=None
        self.__TestResponseStringForStringType = "#HGGMTMOHJHJLJIKTLGLKVUMM" + chr(
            13)  # эта строка будет "читаться" из устройства, если serialPort=None
    ...
    def PackFrame2Ascii(self, hash, request=True):  # формируем строку из фрейма для отправки на устройство
        ...
        self.DebugMessage(
            'Sending::frame size: {0}  addr: {1:#x} hash: {2:#x}'.format(frameSize, self.owenProtocol.address,
                                                                         self.owenProtocol.hash))
    ...
    def GetBCD(self, hashOrName, addrOffset=0, withTime=False, withIndex=False):
        """
        возвращает целочисленный параметр по хэшу или имени,
        addrOffset - смещение относительно базового адреса
        """
        ...
        if self.GetPingPong(hash, addrOffset):
            if withTime or withIndex:
                return self.owenProtocol.unpackBCD()
            else:
                return self.owenProtocol.unpackBCD()['value']
        else:
            raise OwenProtocolError('OwenDevice::data size mismatch!')
...
def test():
    ...
    owen = OwenDevice(None,
                      16)  # создаем устройство без указания порта, это значит, что вместо данных будет приходить тестовая строка
